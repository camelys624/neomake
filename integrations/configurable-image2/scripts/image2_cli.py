...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Configurable image2 generation/edit client")
    ...
    def add_common(subparser: argparse.ArgumentParser) -> None:
        ...
        subparser.add_argument(
            "--base-url",
            default=None,
            help="OpenAI-compatible image API base URL; prefer IMAGE2_BASE_URL env var",
        )
        subparser.add_argument(
            "--model",
            default=DEFAULT_MODEL,
            help="Model name, default: auto selects gpt-image-2-1K/2K/4K from --size",
        )
...
def get_api_key(args: argparse.Namespace) -> str:
    ...
    if not key:
        raise Image2Error(
            "Missing API key. Ask the user for the image API key and pass it via IMAGE2_API_KEY."
        )
    return key


def get_base_url(args: argparse.Namespace) -> str:
    base_url = args.base_url or os.environ.get("IMAGE2_BASE_URL")
    if not base_url:
        raise Image2Error(
            "Missing base URL. Ask the user for the OpenAI-compatible image API base URL and pass it via --base-url or IMAGE2_BASE_URL."
        )
...
def parse_size(size: str) -> tuple[int, int]:
    ...
    if not match:
        raise Image2Error(f"Invalid size for auto model selection: {size}")
    return int(match.group(1)), int(match.group(2))


def resolution_tier_for_size(size: str) -> str:
...
def resolve_model(model: str, size: str) -> str:
    if model.lower() != "auto":
        return model
    tier = resolution_tier_for_size(size)
    return MODEL_BY_RESOLUTION_TIER[tier]
...
def generate_once(args: argparse.Namespace, api_key: str, base_url: str) -> str:
...
def edit_once(args: argparse.Namespace, api_key: str, base_url: str) -> str:
...
def main() -> int:
    ...
    try:
        ...
        args.model = resolve_model(args.model, args.size)
        ...
        if requested_model.lower() == "auto":
            print(f"[image2] auto_model selected from size={args.size}", flush=True)
        print(f"[image2] base_url={base_url}", flush=True)

        for index, path in enumerate(paths, start=1):
            if args.command == "generate":
                b64_data = generate_once(args, api_key, base_url)
            else:
                b64_data = edit_once(args, api_key, base_url)
